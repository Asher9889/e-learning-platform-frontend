...
const TYPE_LABELS: Record<string, string> = {
  VIDEO: "Video", PDF: "PDF", DOCUMENT: "Document", IMAGE: "Image", AUDIO: "Audio",
};
...
export function DraftsTable({ materials, isLoading, onCompleteDetails }: DraftsTableProps) {
  const [previewMaterial, setPreviewMaterial] = useState<Material | null>(null);
  if (isLoading) {
    return <div className="flex items-center justify-center h-48 text-muted-foreground">Loading drafts...</div>;
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-24 text-center">
        ...
        <div className="space-y-1">
          <p className="text-base font-semibold text-foreground">No draft materials</p>
          <p className="text-sm text-muted-foreground">Uploaded files that need metadata will appear here.</p>
        </div>
      </div>
    );
  }
        ...
        <tbody>
          {materials.map((material) => (
            <tr key={material.id} className="border-b last:border-b-0 hover:bg-muted/30">
              ...
              <td className="px-4 py-3 text-muted-foreground">{TYPE_LABELS[material.materialType] || material.materialType}</td>
              <td className="px-4 py-3 text-muted-foreground">{material.createdBy.name}</td>
              <td className="px-4 py-3 text-muted-foreground">{new Date(material.createdAt).toLocaleDateString()}</td>
              ...
              <td className="px-4 py-3 text-right">
                <div className="flex items-center justify-end gap-1">
                  ...
                  <Button variant="outline" size="sm" onClick={() => onCompleteDetails(material)}>
                    Complete Details
                  </Button>
                </div>
              </td>
